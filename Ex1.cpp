...
{
    // Implementar
    Node *currThis = head->next;

    Node *lastDest = dest.last;
    int posThis = 0;
    // int posDest = 0;
    while (currThis != nullptr && currThis->next != nullptr)
    {
        Node *nextCurr = currThis->next;
        currThis->next = nextCurr->next;
        lastDest->next = nextCurr;
        lastDest = nextCurr;
        lastDest->next = nullptr;
...
int main()
{
    int num_casos;
#ifndef DOMJUDGE
    std::ifstream in("samples\\ex2_sample.in");
