...
//
// tuple<int, int, bool> equilibrio(const BinTree<int> &tree)
// {
// }

tuple<int, int, int> nodoHojasAltura(const BinTree<char> &tree)
{
    if (tree.empty())
    {
        return {0, 0, 0};
    ...
    int nodos, nodosDer, nodosIzq;

    tie(nodosDer, hojasDer, alturaDer) = nodoHojasAltura(tree.left());
    tie(nodosIzq, hojasIzq, alturaIzq) = nodoHojasAltura(tree.right());

    hojas = hojasDer + hojasIzq;

    altura = max(alturaDer, alturaIzq) + 1;

    // numero de nodos(formula) = 2^n - 1
    nodos = nodosDer + nodosIzq + 1;

    return tie(nodos, hojas, altura);
}
...
void tratar_caso()
{
    ...
    tuple<int, int, int> resultado = nodoHojasAltura(tree);
    cout << get<0>(resultado) << " " << get<1>(resultado) << " " << get<2>(resultado) << endl;
}

int main()
{
#ifndef DOMJUDGE
    std::ifstream in("samples\\hojas_nodos_altura_sample.in");
