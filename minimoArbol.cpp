...
using namespace std;
template <typename T>
T minTree(const BinTree<T> &tree)
{
    if (tree.left().empty() && tree.right().empty())
    {
        return tree.root();
    }
    T mini, minDer, minIzq;
    minDer = minTree(tree.left());
    minIzq = minTree(tree.right());
    mini = std::min(std::min(minDer, minIzq), tree.root());
    return mini;
}
//
...
// bool primos(const BinTree<int> &tree)
// {
//     // Implementar
//     auto [entero1, entero2, booleano] = equilibrio(tree);
//     return booleano;
// }
...
bool tratar_caso()
{
    ...
    if (c == 'N')
    {
        ...
        cout << minTree<int>(tree) << endl;
    }
    else if (c == 'P')
    {
        BinTree<string> tree = read_tree<string>(std::cin);
        cout << minTree<string>(tree) << endl;
    }
    return true;
}
...
{
#ifndef DOMJUDGE
    std::ifstream in("samples\\minimoArbol_sample.in");
