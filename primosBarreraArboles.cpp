...
pair<int, int> arbol_de_primos(const BinTree<int> &tree)
{
    // Implementar
    return make_pair(get<1>(equilibrio(tree)), get<2>(equilibrio(tree)));
}
...
void tratar_caso()
{
    ...
    if (arbol_de_primos(tree).first == 0 && arbol_de_primos(tree).second == 0)
    {
        cout << "NO HAY" << endl;
    }
    else
    {
        cout << arbol_de_primos(tree).first << " " << arbol_de_primos(tree).second;
    }
...
{
#ifndef DOMJUDGE
    std::ifstream in("samples\\primos_sample.in");
