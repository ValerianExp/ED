...
public:
    void nuevo(const string &dni)
    {
        ...
        // conductores.insert({dni, {info.puntos, puntosConductores[info.puntos].begin()}});
        // puntosConductores.insert({info.puntos, {info.puntos, puntosC2[dni].begin()}});
    }
    ...
    vector<string> lista_por_puntos(int puntos)
    {
        ...
        if (puntosConductores.count(puntos))
        {
            for (auto a : puntosConductores.at(puntos))
            {
                result.push_back(a);
            }
        }
...
{
#ifndef DOMJUDGE
    std::ifstream in("samples\\carnetPorPuntos2_sample.in");
